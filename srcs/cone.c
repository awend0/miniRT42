...
t_solutions	ft_intersect_cone(void *data, t_point origin, t_point dir)
{
	...
	k[0] = ft_vec_dot(dir, dir) - (1.0 + cone->k * cone->k)
		* pow(ft_vec_dot(x, cone->orient), 2.0);
	k[1] = (ft_vec_dot(dir, x) - (1.0 + cone->k * cone->k)
		* ft_vec_dot(dir, cone->orient) * ft_vec_dot(x, cone->orient)) * 2.0;
...
t_point		ft_get_normal_cone(void *data, t_point i)
{
	...
	a = ft_vec_add(cone->p, ft_vec_mul(d, ft_vec_norm(cone->orient)));
