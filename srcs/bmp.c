...
/*   Updated: 2021/03/12 02:06:30 by hasv             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_create_bmp(t_mlxvars *vars)
{
	...
	if (!(fd = open("screenshots/screenshot.bmp", O_CREAT | O_TRUNC | O_WRONLY, 0644)))
	...
	while (i >= 0)
	{
		ret += write(fd, vars->img.addr + (i * (int)g_width * vars->img.bbp / 8), vars->img.line_len);
